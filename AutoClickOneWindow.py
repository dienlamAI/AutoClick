...
def is_click_inside_specific_window(x, y, window_title):
    try:
...
def on_click(x, y, button, pressed):
    global drag_start_pos, drag_in_progress
    window_title = combobox.get()
    if adding_mode and is_click_inside_specific_window(x, y, window_title):
        if pressed:
            drag_start_pos = (x, y)
            drag_in_progress = False
        else:
...
options = [i for i in open_windows if i != '']

option_frame = tk.Frame(root)
option_frame.pack()
option_label = tk.Label(option_frame,text='Lựa chọn cửa sổ: ')
option_label.pack(side=tk.LEFT)
combobox = ttk.Combobox(option_frame, values=options, state='readonly')
combobox.current(0)  
combobox.pack(side=tk.LEFT)

add_frame = tk.Frame(root)
add_frame.pack()
add_click_button = tk.Button(add_frame, text="Thêm Click", command=toggle_adding_mode)
